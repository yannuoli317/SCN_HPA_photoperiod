%% This code explores relationships between distance limit in the %distance-dependt model and R synchronization
clc; clear all; close all;
dmax=[0 0.5 1 1.5 2 2.5 3];
R_rec=zeros(2,length(dmax));
for i=1:length(dmax)
R_rec(:,i)=Rsyn_num(dmax(i))
end
%%
figure
dmax=[0 0.5 1 1.5 2 2.5 3];
...
xlabel('relative threshold distance or probability')
ylabel('Rsyn in the shell')
legend('threshold','probablity')
set(gca,'FontSize',14)
%%
function Rsyn=Rsyn_num(dmax)
...
im=12; A=ode45(@(t,z)gate_oscillator(t,z,im,location,location2,dmax),timeset,B,option);
...
%%
function dzdt=gate_oscillator(t,z,pp,location,location2,dmax)
...
for i=1:ncell2
    ...
    for j=1:ncell %VIP->shell
        if ((location2(i,1)-location(j,1))^2+(location2(i,2)-location(j,2))^2)^0.5<dmax%2.8*(4.6/(pp-3.4))
            VIPsub=z(j*8);
        VIPi=VIPi+VIPsub;
        nact=nact+1;
        end
